import { ADD_PATIENT, FETCH_PATIENT, DELETE_PATIENT, EDIT_PATIENT, PatientsActions, SET_PATIENTS } from './types';
...
export function patientsReducer(
  ...
  switch (action.type) {
    ...
    case FETCH_PATIENT: {
      const fetchPatients = state.map((el) => (el._id !== action.payload._id ? el : action.payload));
